...
def Lagrangian(args):
    i, particle_coords, vel, h, dt, Conc, Conc_N_SPH,vol, NN_idx, DIM  = args
    C_nn = [0, 0]
    if DIM == 2 and (particle_coords[i,0]<0.2 or particle_coords[i,0]>0.8):
        return C_nn
...
def main():
    ...
    while(t<t_end):
        ...
        if single_thread:
            ...
            with mp.Pool(num_processes) as pool:
                ranges = [(i, particle_coords, vel, h, dt, Conc, Conc_N_SPH,vol, NN_idx, DIM) for i in range(num_particles)]
        ...
        count_normal += 1
        t += dt
        print("time = ", f'{t:.3f}', "SPH: ", sum(ConcOrig)-sum(Conc) )
    ...
    header_list = ['x','vel', 'ConcOrig', 'Conc', 'Conc_N_SPH']
    save = np.column_stack((pos, vel,ConcOrig[middle_particles],Conc[middle_particles],Conc_N_SPH[middle_particles]))

    np.savetxt("save_2d.csv", save, delimiter=",", header=','.join(header_list))
