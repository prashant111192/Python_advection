...
def main():
    ...
    vel_avg =   mag
    vel_avg =  np.tile(mag, (len(methods)-1,1))
    v_x = data_last[:,4]
    v_x_avg = np.tile(v_x/len(files), (len(methods)-1,1))
    v_y = data_last[:,5]
    v_y_avg = np.tile(v_y/len(files), (len(methods)-1,1))
    v_z = data_last[:,6]
    v_z_avg = np.tile(v_z/len(files), (len(methods)-1,1))
    ...
    print("saving...")
    np.save(path_to_data+'vel_avg.npy', vel_avg)
    np.save(path_to_data+'v_mag_interpolated_avg.npy', v_mag_interpolated_avg)
