...
methods = ['Cubic Spline', 'Gaussian', 'Quintic Spline']
...
def shepards_method_all_parts(x, y, z, s, u, v, w, NN_idx, h):
    ...
    for i in range(len(u)):
        cs[i], gaussian[i], qs[i] = kernel_methods(x, y, z, s, u[i], v[i], w[i], p, NN_idx[i], h)
    return cs, gaussian, qs
...
def main():
    ...
    for i in range(len(files)-2):
        ...
        cs_v_x, gaussian_v_x, qs_v_x = shepards_method_all_parts(data[:,0], data[:,1], data[:,2], current_v_x, data_last[:,0], data_last[:,1], data_last[:,2], NN_idx, h)
        v_x_avg = v_x_avg+ np.vstack((cs_v_x, gaussian_v_x, qs_v_x))/len(files)

        cs_v_y, gaussian_v_y, qs_v_y = shepards_method_all_parts(data[:,0], data[:,1], data[:,2], current_v_y, data_last[:,0], data_last[:,1], data_last[:,2], NN_idx, h)
        v_y_avg = v_y_avg+ np.vstack((cs_v_y, gaussian_v_y, qs_v_y))/len(files)

        cs_v_z, gaussian_v_z, qs_v_z = shepards_method_all_parts(data[:,0], data[:,1], data[:,2], current_v_z, data_last[:,0], data_last[:,1], data_last[:,2], NN_idx, h)
        v_z_avg = v_z_avg+ np.vstack((cs_v_z, gaussian_v_z, qs_v_z))/len(files)
    ...
    print(np.shape(v_x_avg.T))

    v_x_avg = np.array(v_x_avg, dtype=str)
    v_y_avg = np.array(v_y_avg, dtype=str)
    v_z_avg = np.array(v_z_avg, dtype=str)
    pos = np.array(data_last[:,0:3], dtype=str)
    idp = np.array(data_last[:,3], dtype=int)
    rho = np.array(data_last[:,7], dtype=str)
    type_mk_save = np.array(data_last[:,8:], dtype=int)
    type_mk_save = np.array(type_mk_save, dtype=str)

    cs_save = np.concatenate((pos, np.reshape(idp, (-1,1)), np.reshape(v_x_avg[0].T, (-1,1)), np.reshape(v_y_avg[0].T, (-1,1)), np.reshape(v_z_avg[0].T, (-1,1)), np.reshape(rho, (-1,1)), type_mk_save), axis=1)
    gaussian_save = np.concatenate((pos, np.reshape(idp, (-1,1)), np.reshape(v_x_avg[1].T, (-1,1)), np.reshape(v_y_avg[1].T, (-1,1)), np.reshape(v_z_avg[1].T, (-1,1)), np.reshape(rho, (-1,1)), type_mk_save), axis=1)
    qs_save = np.concatenate((pos, np.reshape(idp, (-1,1)), np.reshape(v_x_avg[2].T, (-1,1)), np.reshape(v_y_avg[2].T, (-1,1)), np.reshape(v_z_avg[2].T, (-1,1)), np.reshape(rho, (-1,1)), type_mk_save), axis=1)
    # cs_save = np.concatenate((data_last[:,0:3], np.reshape(v_x_avg[0].T, (-1,1)), np.reshape(v_y_avg[0].T, (-1,1)), np.reshape(v_z_avg[0].T, (-1,1)), data_last[:,6:8]), axis=1)
    # gaussian_save = np.concatenate((data_last[:,0:3], np.reshape(v_x_avg[1].T, (-1,1)), np.reshape(v_y_avg[1].T, (-1,1)), np.reshape(v_z_avg[1].T, (-1,1)), data_last[:,6:8]), axis=1)
    # qs_save = np.concatenate((data_last[:,0:3], np.reshape(v_x_avg[2].T, (-1,1)), np.reshape(v_y_avg[2].T, (-1,1)), np.reshape(v_z_avg[2].T, (-1,1)), data_last[:,6:8]), axis=1)
    # cs_save = np.array(cs_save, dtype=str)
    # gaussian_save = np.array(gaussian_save, dtype=str)
    # qs_save = np.array(qs_save, dtype=str)
    # cs_save = np.concatenate((cs_save, type_mk_save), axis=1)
    # gaussian_save = np.concatenate((gaussian_save, type_mk_save), axis=1)
    qs_save = np.concatenate((qs_save, type_mk_save), axis=1)
    np.savetxt(path_to_output+'cs.csv', cs_save, delimiter=';', fmt='%s', header='x;y;z;v_x;v_y;v_z;rho;type;mk')
    np.savetxt(path_to_output+'gaussian.csv', gaussian_save, delimiter=';', fmt='%s', header='x;y;z;v_x;v_y;v_z;rho;type;mk')
    np.savetxt(path_to_output+'qs.csv', qs_save, delimiter=';', fmt='%s', header='x;y;z;v_x;v_y;v_z;rho;type;mk')
