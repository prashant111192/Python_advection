...
def main():
    ...
    data_last = np.uint64(data_last)
    mag = np.uint64(mag)
    print(data_last.flags)
    print(mag.flags)
    data_last = np.reshape(data_last, (len(data_last),10), order ='C')
    print(np.shape(data_last))
    print(data_last.flags)

    pointsToVTK('./VTK/vel_avg', data_last[:,0], data_last[:,1], data_last[:,2], data = {'vel_avg': mag
    })
